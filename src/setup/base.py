    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.user_id:
            await interaction.response.send_message(
                "(´・ω・`) No tienes permiso para interactuar con este menú.",
                ephemeral=True
            )
            return False
        return True
...
class SetupManager:
    ...
    def register_module(self, name: str, view_factory: Callable[[int, int], discord.ui.View]) -> None:
    ...
    def get_view(self, name: str, guild_id: int, user_id: int) -> Optional[discord.ui.View]:
