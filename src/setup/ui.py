...
class SetupDropdown(discord.ui.Select):
    def __init__(self, setup_manager: SetupManager, guild_id: int, user_id: int):
        ...
        if not modules:
            options.append(discord.SelectOption(label="Sin módulos", description="No hay módulos configurables aún."))
        else:
            for module_name in modules:
                options.append(
                    discord.SelectOption(label=module_name, description=f"Configurar opciones de {module_name}")
                )
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if view:
            ...
            await interaction.response.send_message("(´・ω・`) No se pudo cargar el módulo.", ephemeral=True)